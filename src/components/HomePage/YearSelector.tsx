...
export default function YearSelector() {
  ...
  return (
    <div className="relative flex flex-col mt-6 pb-8">
      <div className=" flex flex-row gap-6 items-center overflow-x-auto max-w-sm [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none] overscroll-x-contain touch-pan-x">
        {savedYears.map((year) => {
          return (
            <ul key={nanoid()}>
              <li
        ...
        >
          {remainingYears.map((year) => (
            <ul key={nanoid()} className="mt-2 mr-2">
              <li
                key={year}
